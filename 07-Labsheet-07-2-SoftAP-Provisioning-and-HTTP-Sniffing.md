...
| :--- | :--- |
| **1. ชื่อ SoftAP SSID ของ ESP32** | PROV_AE1D88 |
...
```
สื่อสารผ่านโปรโตคอล HTTP (ผ่าน Protocomm transport layer ทาง Endpoints ต่างๆ เช่น /prov-session, /prov-config) โดยปกติเมื่อ ESP32 กระจายสัญญาณเป็น Access Point จะกำหนดหมายเลข Gateway / IP Address ของตัวเองเป็น 192.168.4.1 สมาร์ตโฟนจึงต้องยิง HTTP POST Requests มายัง IP 192.168.4.1 นี้
```
...
```
ESP32 จะนำ Credentials ที่ได้รับไปลองเชื่อมต่อกับ Router เมื่อล้มเหลวจะเกิด Event WIFI_EVENT_STA_DISCONNECTED ขึ้นก่อน จากนั้นระบบ Provisioning Manager จะตรวจพบและ Trigger Event WIFI_PROV_CRED_FAIL (พร้อมเหตุผล WIFI_PROV_STA_AUTH_ERROR) 

พฤติกรรมของ ESP32 คือ จะยังไม่ปิดโหมด SoftAP แต่จะส่งข้อความแจ้งเตือน Error Status กลับไปยังแอปพลิเคชันบนมือถือ เพื่อให้หน้าจอแอปขึ้นเตือนรหัสผ่านไม่ถูกต้อง และรอให้ผู้ใช้ป้อนรหัสผ่านใหม่อีกครั้ง
```
3. ทำไมผู้ผลิต IoT ส่วนใหญ่จึงมองว่ากระบวนการเชื่อมต่อแบบ SoftAP มีขั้นตอนที่ยุ่งยากสำหรับผู้ใช้ทั่วไปเมื่อเทียบกับ BLE?
```
การใช้ SoftAP บังคับให้ผู้ใช้ต้องสลับการเชื่อมต่อ Wi-Fi บนสมาร์ตโฟนออกจาก Router บ้านไปสแกนจับ Wi-Fi ของตัวอุปกรณ์ IoT (PROV_XXXXXX) ซึ่งระบบปฏิบัติการบนมือถือ (ทั้ง iOS และ Android) มักจะมีป๊อปอัปแจ้งเตือนว่า "Wi-Fi นี้ไม่มีอินเทอร์เน็ต" และพยายามตัดกลับไปใช้ Cellular Data / 4G / 5G ทำให้อุปกรณ์หลุดการสื่อสารได้ง่าย 

ในขณะที่ BLE (Bluetooth Low Energy) สามารถค้นหา จับคู่ และส่งข้อมูล Wi-Fi Credentials ข้ามไปยัง ESP32 ได้ทันทีในฉากหลัง (Background Process) โดยที่ผู้ใช้ไม่ต้องหลุดออกจาก Wi-Fi บ้านเดิมเลย จึงให้ประสบการณ์ใช้งาน (UX) ที่ราบรื่นและมีโอกาสผิดพลาดน้อยกว่ามาก
```
